...
namespace DonVo.WebMVC.Controllers.Directories
{
    ...
    {
        private readonly IAccountingService _services;
        ...
        {
            var result = await _services.GetAccounting(Convert.ToInt32(id));
            if (result.IsValid)
            {
                return View(result.Result);
            }
        ...
        public async Task<IActionResult> CreateConfirmed(CreateMainAccountingViewModel vm)
        {
            ...
            {
                var result = await _services.CreateMainAccountingAsync(_mapper.Map<CreateAccountingDTO>(vm));
                if (result.IsValid)
                {
                    await _systemAuditService.AuditAsync(User.GetEmail(), _accessor.GetIp(), Operations.Insert, Tables.DimAccount);
        ...
        public async Task<IActionResult> Delete([Required] string id)
        {
            ...
            if (result.IsValid)
            {
                await _systemAuditService.AuditAsync(User.GetEmail(), _accessor.GetIp(), Operations.Delete, Tables.DimAccount);
            }
            return Ok(result);
        }
